...
const load = async () => {
  ...
  } catch (cause) { error.value = getErrorMessage(cause, '结算信息加载失败，请稍后重试') }
...
const submit = async () => {
  ...
  if (!items.value.length) return ElMessage.warning('请先选择需要结算的商品')
  ...
  try {
    ...
    await removeCartBatch(items.value.map(i => i.id)).catch(() => undefined)
    ElMessage.success('下单成功，正在打开订单详情')
    await router.replace(`/order/${r.data.data.orderNo}`)
  } catch {}
  finally { submitting.value = false }
}
</script>
<template>
  <div v-loading="loading" style="max-width:800px;min-height:320px;margin:0 auto">
    <div class="section-title">确认订单</div>
    <el-alert v-if="error" :title="error" type="error" show-icon :closable="false" style="margin-bottom:14px"><template #default><el-button size="small" @click="load">重新加载</el-button></template></el-alert>
    <el-empty v-else-if="!loading && !items.length" description="没有已选择的结算商品"><el-button type="primary" @click="router.push('/cart')">返回购物车</el-button></el-empty>
    ...
    <div style="background:#fff;padding:16px;margin-bottom:14px"><div style="font-weight:600;margin-bottom:8px">商品清单</div>
      ...
      <div style="text-align:right;margin-top:14px;line-height:1.8"><div>商品金额 ¥{{ total().toFixed(2) }}</div><div>配送费用 ¥{{ shippingFee().toFixed(2) }}</div><span>合计 <span style="font-size:22px" class="price">¥{{ (total() + shippingFee()).toFixed(2) }}</span></span><el-button size="large" :disabled="submitting" @click="router.push('/cart')" style="margin-left:12px">返回购物车</el-button><el-button type="danger" size="large" :loading="submitting" :disabled="!shippingRules.length" @click="submit" style="margin-left:8px">提交订单</el-button></div>
    </div>
    </template>
  </div>
</template>
