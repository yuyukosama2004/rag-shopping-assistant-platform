...
const timeline = () => [
  { label: '已下单', time: order.value.createdAt },
  order.value.payTime && { label: order.value.paymentMethod === 'COD' ? '已确认货款' : '已确认收款', time: order.value.payTime },
  order.value.processingAt && { label: '商家已接单', time: order.value.processingAt },
  order.value.shippedAt && { label: '已发货', time: order.value.shippedAt },
  order.value.status === 3 && { label: '已完成', time: order.value.updatedAt },
].filter(Boolean)
</script>
<template>
  <div v-if="order" style="max-width:800px;margin:0 auto">
    ...
    <div style="background:#fff;padding:20px;margin-bottom:14px">
      ...
      <el-timeline style="margin-top:18px"><el-timeline-item v-for="item in timeline()" :key="item.label" :timestamp="item.time" type="primary">{{ item.label }}</el-timeline-item></el-timeline>
