...
import { getOrderDetail, payOrder, cancelOrder, completeOrder } from '../api/order'; import { ElMessage, ElMessageBox } from 'element-plus'
...
<template>
  <div v-if="order" style="max-width:800px;margin:0 auto">
    ...
    <div style="background:#fff;padding:20px;margin-bottom:14px">
      ...
      <div style="font-size:13px;color:#555;line-height:2">收货人: {{ order.receiverName }} {{ order.receiverPhone }}<br/>地址: {{ order.receiverAddress }}<br/>下单: {{ order.createdAt }}<br/>支付: {{ order.payTime||'-' }}<template v-if="order.status >= 2"><br/>物流: {{ order.shippingCarrier || '-' }} {{ order.trackingNo || '-' }}<br/>发货: {{ order.shippedAt || '-' }}</template></div>
