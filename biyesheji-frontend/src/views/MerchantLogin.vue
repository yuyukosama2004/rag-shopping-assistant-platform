...
const handle = async () => {
  try {
    ...
    if (userStore.user?.role !== 1 && userStore.user?.role !== 2) {
      userStore.clearSession()
      ElMessage.error('此账号没有商家工作台权限')
      return
    }
...
<template>
  <div style="display:flex;min-height:100vh;align-items:center;justify-content:center;background:#f3f4f6">
    <div style="width:400px;background:#fff;padding:36px;border-radius:8px;box-shadow:0 8px 24px rgb(0 0 0 / 8%)">
      ...
      <p style="margin:0 0 24px;color:#6b7280">仅限本店店主或店员登录</p>
      <el-form :model="form" size="large" @submit.prevent="handle">
        <el-form-item><el-input v-model="form.username" autocomplete="username" placeholder="商家账号" /></el-form-item>
