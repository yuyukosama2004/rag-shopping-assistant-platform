...
<template>
  <el-card v-loading="loading">
    ...
    <el-table :data="orders">
      ...
      <el-table-column label="配送" min-width="130"><template #default="{ row }"><div>{{ row.shippingRuleName || (row.shippingMethod === 'PICKUP' ? '门店自提' : '配送') }}</div><div style="font-size:12px;color:#909399">商品 ¥{{ Number(row.productAmount ?? row.totalAmount).toFixed(2) }} + 配送 ¥{{ Number(row.shippingFee || 0).toFixed(2) }}</div></template></el-table-column>
