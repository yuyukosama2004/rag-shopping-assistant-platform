...
public class OrderController {
    ...
    @Operation(summary = "创建测试订单（开发调试用）")
    @PostMapping("/test-create")
    public R<String> testCreate(@RequestHeader("Authorization") String auth,
                                 @RequestBody Map<String, Object> body) {
        Long productId = Long.valueOf(body.get("productId").toString());
        Integer quantity = body.containsKey("quantity")
                ? Integer.valueOf(body.get("quantity").toString()) : 1;
        return R.ok(orderService.createTestOrder(getUserId(auth), productId, quantity));
    }
}
