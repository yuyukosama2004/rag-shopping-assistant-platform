...
public class MerchantOrderNotificationController {
    private final OrderNotificationOutboxService service;

    @GetMapping
    public R<List<OrderNotificationOutbox>> list(@RequestHeader("X-User-Role") Integer role) {
    ...
    private void requireMerchant(Integer role) {
        if (!Integer.valueOf(UserRole.OWNER).equals(role) && !Integer.valueOf(UserRole.STAFF).equals(role)) {
            throw new BizException(403, "仅商家可查看订单通知");
        }
    }
}
