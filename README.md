...
当前版本处于 Alpha 阶段，重点验证电商交易链路、微服务治理与 RAG 智能导购在自托管环境中的完整集成。

## 项目状态

- 阶段：Alpha，持续开发
- 部署：自托管环境
- 范围：商城交易链路、微服务治理、RAG 智能导购
...
```bash
./start.sh install v0.6.0-alpha.1
```

升级前先将仓库切换到目标 Tag/提交，再执行：

```bash
./start.sh upgrade v0.7.0-alpha.1
```
...
./start.sh rollback v0.6.0-alpha.1  # 指定已保留版本
```
...
本项目是持续开发中的个人全栈 AI 应用，当前以 Alpha 版本提供自托管部署能力。运行 AI 功能时，第三方模型接口可能产生费用，请根据对应平台的计费规则合理使用。
