...
系统实现了用户注册登录、商品查询、购物车、收货地址、异步下单、库存管理、模拟支付、订单管理等主要电商功能。同时接入大语言模型与向量检索能力，为用户提供基于商品数据的 AI 智能导购服务。
...
├── order-service/             # 购物车、订单、库存、RabbitMQ 和 AI 导购
├── biyesheji-frontend/        # Vue 3 前端项目
├── sql/
│   ├── init.sql               # 数据库表结构
│   └── mock_data.sql          # 示例商品与业务数据
├── docker/                    # MySQL、Redis、RabbitMQ、Nacos 编排文件
...
| Redis | 6379 | 缓存服务 |
| RabbitMQ | 5672 | 消息服务 |
| RabbitMQ 管理页面 | 15672 | RabbitMQ Web 控制台 |
| Nacos | 8848 | 注册中心 |
...
项目提供了 MySQL、Redis、RabbitMQ 和 Nacos 的 Docker Compose 配置：
...
| 命令 | 说明 |
| --- | --- |
| `infra-start` | 启动 MySQL、Redis、RabbitMQ 和 Nacos |
| `infra-stop` | 停止基础设施 |
| `build` | 构建全部后端模块 |
| `start` | 启动四个微服务 |
| `stop` | 停止全部微服务 |
