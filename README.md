...
当前发布版本为 `v0.8.0-alpha.3`。该版本在既有服务器升级、备份、监控和回滚验收基础上，
新增只读 AI 检索评测端点、确定性离线评测和回归任务；真实商家试点、公网域名 TLS、
外部告警送达和真实支付仍未完成，因此继续使用 Alpha 标识。

## 项目状态

- 发布：[`v0.8.0-alpha.3`](https://github.com/yuyukosama2004/rag-shopping-assistant-platform/releases/tag/v0.8.0-alpha.3)
- 阶段：Alpha，已达到单商家非生产数据试点候选
- 部署：单商家、单店铺、单实例自托管
- CI：GitHub 托管 `ubuntu-latest`，执行 14 项任务；当前 13 个上下文为 `main` 必需检查
...
```bash
./start.sh install v0.8.0-alpha.3
```

升级前先将仓库切换到目标 Tag/提交，再执行：

```bash
./start.sh upgrade v0.8.0-alpha.3
```
...
- [`docs/AI评测接口.md`](docs/AI评测接口.md)
- [`docs/AI经营与库存助手实施计划.md`](docs/AI经营与库存助手实施计划.md)
- [`docs/AI经营与库存助手开发交接.md`](docs/AI经营与库存助手开发交接.md)
- [`evals/README.md`](evals/README.md)

### GitHub CI 与分支保护

`.github/workflows/ci.yml` 固定使用 GitHub 托管的 `ubuntu-latest` Runner，不要求部署服务器承担
CI。Pull Request 和 `main` 推送会执行 14 项检查，包括离线确定性评测、后端、前端、配置、
完整 Playwright E2E、源码密钥/依赖扫描、四个业务镜像以及基础设施和监控镜像扫描。

`main` 已启用严格分支保护：当前 13 个必需检查必须全部通过、分支必须保持线性历史、讨论必须解决，
并禁止强制推送和删除。`evaluation` 已作为第 14 项任务执行，待评测基线稳定后加入必需检查。
X99 等部署服务器只负责开发构建和版本部署，不注册 GitHub self-hosted Runner。
...
受保护环境可选择启用默认关闭的商家只读检索评测端点：

```text
POST /api/merchant/ai/evaluate
```

其安全边界、配置和响应语义见 [`docs/AI评测接口.md`](docs/AI评测接口.md)。
...
本项目是持续开发中的个人全栈 AI 应用，当前以 `v0.8.0-alpha.3` 提供自托管部署能力。
